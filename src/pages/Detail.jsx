...
const Detail = () => {
  ...
  useEffect(() => {
    size == "L"
      ? setAvailableSize(detail?.sizes.L)
      : size == "M"
      ? setAvailableSize(detail?.sizes.M)
      : size == "S"
      ? setAvailableSize(detail?.sizes.S)
      : size == "XL"
      ? setAvailableSize(detail?.sizes.XL)
      : size == "twoXL"
      ? setAvailableSize(detail?.sizes.twoXL)
      : size == "threeXL"
      ? setAvailableSize(detail?.sizes.threeXL)
      : null;
  }, [size, detail]);
  ...
  const addToCart = async () => {
    ...
    if (!token) {
      ...
      localStorage.setItem(
        ...
        JSON.stringify({
          type: "error",
          value: "Please log in before adding items to your cart.",
        })
      );
      return;
    }
    try {
      setLoading(true);
      const url =
        "https://reborn-necessary-clothing-backend.onrender.com/api/products/add-to-cart";
      const theHeaders = {
        headers: {
      ...
      localStorage.setItem(
        "user",
        JSON.stringify({ ...user, cart: response.data.cart })
      );
      setLoading(false);
      console.log(response);
      navigate("/");
      ...
      <div className="detail_body">
        <div className="detail_image_section">
          <div className="detail_image_main_display">
            <img
              src={!displayImage ? detail?.images[0] : displayImage}
              alt="img"
            />
          </div>
            ...
            <div className="detail_description_container_last">
              ...
              <div className="add_to_cart_btn_container">
                <button onClick={addToCart}>
                  ...
                  {loading ? (
                    <LiaSpinnerSolid className="add_to_cart_spinner" />
                  ) : (
                    <p>Add To Cart</p>
                  )}
                </button>

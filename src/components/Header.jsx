    ...
    <>
      <div className="header_body">
        ...
        <div className="header_nav_container">
          <Link to="/designer-mood" className="header_nav">
            Designer Mood
          </Link>
          <Link to="/product-category/women" className="header_nav">
            For Her
          </Link>
          <Link to="/product-category/men" className="header_nav">
            For Him
          </Link>
          <Link to="/product-category/accessories" className="header_nav">
            Accessories
          </Link>
        </div>
        <div className="header_account_container">
          ...
          <FaUserLarge
            className="cart_icon"
            cursor="pointer"
            onClick={() => setShowAccntMenu(!showAccntMenu)}
          />
          {showAccntMenu == true ? (
            <div className="account_selection_container">
              {userActive == true ? null : (
                <button onClick={() => navigate("/login")}>Sign In</button>
              )}
              <div className="account_selection_link" onClick={gotToAccount}>
                {" "}
                <FaRegUser className="asl_icon" /> My Account
              </div>
              <div className="account_selection_link" onClick={goToOrders}>
                {" "}
                <BiPackage className="asl_icon" /> Orders
              </div>
              {userActive == true ? (
                <div className="account_selection_logout_link" onClick={logout}>
                  Logout
                </div>
              ) : null}
            </div>
        ...
        >
          ...
          <div
            onClick={gotToAccount}
            to="/account"
            className="mobile_header_nav"
          >
            My Account
          </div>
          <div onClick={goToOrders} to="/orders" className="mobile_header_nav">
            Orders
          </div>
          {userActive == true ? (
            <Link onClick={logout} className="mobile_header_nav">
              Logout
            </Link>
          ) : (
            <Link
